...
def do_roll() -> int:
    with tracer.start_as_current_span("master") as master:
        with tracer.start_as_current_span("do_compute") as compute_span:
            response = get("http://localhost:8081/doCompute")
